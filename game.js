...
function create() {
    ...
    soundToggle = game.add.button(game.world.width - 130, 15, 'mute', setSound, this);
    soundToggle.frame = 0;
...
function update() {
    ...
    if(level>2)
        theBackground.tilePosition.y += 2;
        ...
        for (var i = 0; i < levels[level]; i++) {
            if (balls[i] !== null && !lost) {
                ...
                if (balls[i].alive) {
                    ...
                    if(level>1)
                        balls[i].rotation += balls[i].body.velocity.x / 500;
                    game.physics.arcade.collide(balls[i], platforms, collisionHandler, null, this);
                    if(i>0)
                        game.physics.arcade.collide(balls[i], balls[i-1]);
                }
        ...
        if (allBallsKilled) {
            ...
            if (level < 5) {
                ...
                if (seconds > time_intervals[level]) {
                    ...
                    scoreText.font.blink();
                    if (sound){
                        gameover.play();
                    }
                    stateText.text = " You Lost!";
                    stateText.visible = true;
                    restartGame = game.add.button(game.world.width / 2 + 10, 195, 'restart', this.restart, this);
                    restartGame.anchor.setTo(0.5, 0.5);
                    restartGame.alpha = 0;
                    game.add.tween(restartGame).delay(2000).to({
                        alpha: 1
                    }, 1000).start();
                    shakeIt(20, 100);
                    lost = true;
...
function collisionHandler(ball, ledge) {
...
function nextLevel() {
    ...
    for (var i = 0; i < levels[level]; i++) {
        ...
        balls[i] = game.add.sprite(game.world.centerX + Math.random() * 30, game.world.centerY + Math.random() * 30, 'ball');
        ...
        balls[i].body.velocity.x = (Math.random() < 0.5 ? -1 : 1) * (ballSpeed + Math.random() * 5);
        balls[i].body.velocity.y = (Math.random() < 0.5 ? -1 : 1) * (ballSpeed + Math.random() * 5);
        balls[i].angle = 90 + Math.atan2(yy - toy, xx - tox) * 180 / Math.PI;
        balls[i].body.collideWorldBounds = true;
        if(level>0)
            balls[i].body.bounce.setTo(0.7 + Math.random() * 0.2, 0.7 + Math.random() * 0.2);        
...
function updateTimer() {
    ...
    timer.setText(minutes + ':' + seconds + ':' + ("" + milliseconds).substring(0, 2));
